...
class MessageCriteria {
    ...
    Date date
    /** Indique l'opérateur utilisé pour effectuer la comparaison de date (cf {@link URLParamsValue}) :  utilisation conjointe avec {@link #date} */
    String operator

    MessageCriteria(String count, String date, String operator) {
        this.count = count != null ? new Long(count) : 0
        this.date = date != null ? URLParamsValue.DATE_FORMAT.parse(date) : null
